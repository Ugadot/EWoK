...
for task in ALL_TASKS:
    ...
    rewards = []
    folder = f"./continuous_results/td3/noise_change/{task}"
    ...
    rewards = []
    folder = f"./continuous_results/td3/noise_change/{task}_reset"
    runs = sorted(os.listdir(folder))
    for run in runs:
        rewards_per_run = []
        df = pd.read_csv(f"{folder}/{run}/eval.csv")
    ...
    # Collect results for Domain Rand
    rewards = []
    folder = f"./continuous_results/td3/noise_change/{task}_dr"
    runs = sorted(os.listdir(folder))
    for run in runs:
        rewards_per_run = []
        df = pd.read_csv(f"{folder}/{run}/eval.csv")
        for step, value in enumerate(param_values):
            ...
            rewards_per_run.append(reward)
        rewards.append(np.asarray(rewards_per_run))
    dmn_rand_rewards = np.asarray(rewards)
    ...
    for i in range(len(param_values)):
        result_dict = {"our": our_rewards[:, [i]], "baseline": baseline_rewards[:, [i]],
                       "dmn_rand": dmn_rand_rewards[:, [i]]}
        scores, score_cis = rly.get_interval_estimates(
            result_dict, metrics.aggregate_iqm, reps=10000)

        dmn_rand_iqm.append((score_cis['dmn_rand'][0, 0], scores['dmn_rand'], score_cis['dmn_rand'][1, 0]))
    ...
    dmn_rand_iqm = np.asarray(dmn_rand_iqm)
    ...
    # Plot baseline
    plt.plot(param_values, baseline_iqm[:, 1], f'{BASELINE_SHAPE}-', color=BASELINE_COLOR, label='TD3',
    ...
    # Plot dmn_rand
    plt.plot(param_values, dmn_rand_iqm[:, 1], f'{DMN_RAND_SHAPE}-', color=DMN_RAND_COLOR, label='Domain Rand.',
             markersize=MARKER_SIZE, linewidth=LINE_WIDTH)
    plt.fill_between(param_values, dmn_rand_iqm[:, 0], dmn_rand_iqm[:, 2],
                    linewidth=0, color=DMN_RAND_COLOR, alpha=ALPHA_LEVEL)
    min_y = min(min_y, min(dmn_rand_iqm[:, 0]))
    max_y = max(max_y, max(dmn_rand_iqm[:, 2]))
    min_x = min(param_values)
    max_x = max(param_values)

    save_path = f"./figs/td3/{task}"
    ...
    for param, values in perturbations.items():
        ...
        # Collect results for baseline
        rewards = []
        folder = f"./continuous_results/td3/param_change/{task}/{param}"
        runs = sorted(os.listdir(folder))
        for run in runs[-10:]:
            df = pd.read_csv(f"{folder}/{run}/eval.csv")
        ...
        # Collect results for our method
        rewards = []
        folder = f"./continuous_results/td3/param_change/{task}_reset/{param}"
        runs = sorted(os.listdir(folder))
        for run in runs[-10:]:
            df = pd.read_csv(f"{folder}/{run}/eval.csv")
            rewards.append(df['episode_reward'].to_numpy())
        ...
        # Collect results for domain rand
        rewards = []
        folder = f"./continuous_results/td3/param_change/{task}_dr/{param}"
        runs = sorted(os.listdir(folder))
        for run in runs:
            rewards_per_run = []
            df = pd.read_csv(f"{folder}/{run}/eval.csv")
        ...
        # Plot baseline
        plt.plot(values, baseline_iqm[:, 1], f'{BASELINE_SHAPE}-', color=BASELINE_COLOR, label='TD3',
                 markersize=MARKER_SIZE, linewidth=LINE_WIDTH)
        ...
        # Plot Domain Rand
        plt.plot(values, dmn_rand_iqm[:, 1], f'{DMN_RAND_SHAPE}-', color=DMN_RAND_COLOR, label='Domain Rand.',
                 markersize=MARKER_SIZE, linewidth=LINE_WIDTH)
        plt.fill_between(values, dmn_rand_iqm[:, 0], dmn_rand_iqm[:, 2],
                        linewidth=0, color=DMN_RAND_COLOR, alpha=ALPHA_LEVEL)
        min_y = min(min_y, min(dmn_rand_iqm[:, 0]))
        max_y = max(max_y, max(dmn_rand_iqm[:, 2]))
        min_x = min(values)
