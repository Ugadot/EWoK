...
from sb3_extra.ormdp_dqn import ORMDP_DDQN
from sb3_extra.dom_rand_dqn import DOM_RAND_DQN
...
def main(args):
    ...
    if args.load is False:
        ...
        eval_callback = EvalCallback(eval_env, best_model_save_path=f"{model_path}", n_eval_episodes=20,
                                     log_path=f"./logs/eval/{args.env_name}/{nominal_vals_text}/{algo_text}/seed_{args.train_seed}", eval_freq=1_000,
...
def get_params():
    ...
    parser.add_argument('-a', '--algo', type=str, choices=['Ours', 'Vanilla', 'Domain_Rand'])
